...
public class notificationClientController implements Initializable{
	private DialogPane dialog;

	@FXML
	private Button btn_information;
	Label SPNameLabel = new Label("");
	

    String acceptedOrRefusedT;
	String dateT;
	String timeT;
	String SPNameT;
	String contentT;
	...
	public int selectIdClient() {

		DatabaseConnection connectNow = new DatabaseConnection();
		Connection connect = connectNow.getConnection();

		String selectId = "SELECT iduser FROM user WHERE username = ? and password = ?";
		String name = MyAppContext.ClientUsername;
		String password = MyAppContext.ClientPassword;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		SelectDataFromNotificationFirstTime();
		SelectDataFromNotification();
	}
	public void SelectDataFromNotification() {	
		//this is a time listener that refreshes the listView from database every 2 seconds .
		Timeline refreshTimeline = new Timeline(new KeyFrame(Duration.seconds(2), new EventHandler<ActionEvent>() {
		    @Override
		    public void handle(ActionEvent event) {
		ObservableList<GridPane> postGrids = FXCollections.observableArrayList();
		DatabaseConnection connectNow = new DatabaseConnection();
		Connection connect = connectNow.getConnection();
		String selectData = "SELECT  * FROM notificationclient WHERE destiId=?";
		try {
			PreparedStatement st = connect.prepareStatement(selectData);
			st.setInt(1, selectIdClient());
            System.out.println("i m here"+selectIdClient());
			ResultSet result = st.executeQuery();

			while (result.next()) {
				SPNameT = result.getString("SPname");
			    dateT = result.getString("createdAtDate");
			    timeT = result.getString("createdAtTime");
			    int idNot = result.getInt("idNotifi");
			   
			   acceptedOrRefusedT = result.getString("acceptedOrRefused");

		
			
			Label dateLabel = new Label("Reservation Time : " + dateT + " at : " + timeT);
			
		 SPNameLabel = new Label(SPNameT);
			Label contentLabel = new Label( acceptedOrRefusedT+" your reservation .");
		        
		SPNameLabel.setId("SPname");
			    dateLabel.setStyle("-fx-text-fill: gray;-fx-font-size:12px;");
				SPNameLabel.setStyle("-fx-font-weight: bold; -fx-font-size:14px;-fx-text-fill: #2c7a6a;");
				...
				postGrid.setConstraints(SPNameLabel, 1, 0, 1, 1);
			...
			postGrid.getChildren().addAll(SPNameLabel, dateLabel, contentLabel);
		        ...
		        if (event.getButton().equals(MouseButton.PRIMARY)) {
		            if (event.getClickCount() == 2) {
		            	GridPane selectedGrid = (GridPane) listView.getSelectionModel().getSelectedItem();
		            	 SPNameLabel = (Label) selectedGrid.lookup("#SPname");
		            	String clientName = SPNameLabel.getText();
		            	System.out.println("client who reserved :"+clientName);
                       MyAppContext.ClientWhoReserved=clientName;
		                
		                try {
		        			Parent parent;
		        			parent = FXMLLoader.load(getClass().getClassLoader().getResource("src/View/printReser.fxml"));
	...
	public void SelectDataFromNotificationFirstTime() {	
		ObservableList<GridPane> postGrids = FXCollections.observableArrayList();
		DatabaseConnection connectNow = new DatabaseConnection();
		Connection connect = connectNow.getConnection();
		String selectData = "SELECT  * FROM notificationclient WHERE destiId=?";
		try {
			...
			while (result.next()) {
			   ...
			   acceptedOrRefusedT = result.getString("acceptedOrRefused");
System.out.println("accepted or no :"+acceptedOrRefusedT);
		if(acceptedOrRefusedT=="accepted") {
			contentT="has accepted your reservation .";
		}else if(acceptedOrRefusedT=="refused"){
			contentT="has refused your reservation .";
		}
			
			Label dateLabel = new Label("Reservation Time : " + dateT + " at : " + timeT);
			SPNameLabel.setText(SPNameT);
		...
		postGrids.addListener(new ListChangeListener<GridPane>() {
		    @Override
		    public void onChanged(Change<? extends GridPane> c) {
		        MyAppContext.getInstance().setListModified(true);
		    }
		});
